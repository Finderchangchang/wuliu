 ...
 */
public class CheckConfigTask extends HttpTask{
    private CheckConfigListener mListener;
    ...
    public void start(CheckConfigListener listener) {
        mListener = listener;
        if (mListener != null) {
        ...
        if (mListener != null) {
            if (result) {
                if (object != null) {
                    ...
                    try {
                        model = HttpXml.parseXml((InputStream) object, "TerminalModel");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
